(function($) {
        ...
        menuSidebarMobile: function() {
            ...
            const menuSidebarMobileOpen = () => {
                ...
                if (window.mobile_menu == "default") {
                    if (!$("#navigation-mobile .site-nav-mobile.nav .header__inline-menu")
                        .length
                    ) {
        ...
        initQuickShop: function() {
            if (window.quick_shop.show) {
                $doc.on("click", "[data-quickshop-popup]", (event) => {
                    ...
                    if (!document.body.matches(".qv-loaded", ".qs3-loaded") &&
                        $quickView
                    ) {
                            ...
                            case "quick_cart":
                                if (window.quick_cart.show) {
                                    Shopify.getCart((cart) => {
                                        if (window.quick_cart.type == "popup") {} else {
    ...
    (async function detectCurrencyByIP() {
        ...
        try {
            ...
            console.log("📡 IP Detected:", data.country_name, data.currency);

            if (
                data.continent_code === "EU" &&
                !supportedCurrencies.includes(detectedCurrency)
            ) {
                detectedCurrency = "EUR";
                console.log("🇪🇺 Fallback to EUR");
            }

            if (!supportedCurrencies.includes(detectedCurrency)) {
                detectedCurrency = fallbackByLanguage() || "INR";
                console.log("🔁 Final fallback:", detectedCurrency);
            }
        } catch (err) {
            console.warn("⚠️ IP detection failed — fallback to language", err);
            detectedCurrency = fallbackByLanguage() || "INR";
        }
        ...
        if (currencySelector && currencySelector.value !== detectedCurrency) {
            currencySelector.value = detectedCurrency;
            currencySelector.dispatchEvent(new Event("change", {
                bubbles: true
            }));

            const customDropdown = document.getElementById("currencySelected");
            const selectedLi = document.querySelector(
                `li[data-code="${detectedCurrency}"]`
            );
            if (customDropdown && selectedLi) {
                const flag = selectedLi.dataset.flag;
                customDropdown.innerHTML = `<img src="${flag}" alt="${detectedCurrency}"><span>${detectedCurrency}</span>`;
            }

            console.log("✅ Currency set:", detectedCurrency);
        }
    })();
