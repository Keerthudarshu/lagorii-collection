class PredictiveSearch extends HTMLElement {
    ...
    setupEventListeners() {
        ...
        this.input.addEventListener('click', this.doBeforeQuickSearchProduct.bind(this));
        if (window.innerWidth <= 1024) {
            if (document.querySelector('[data-search-mobile]')) {
                document.querySelectorAll('[data-search-mobile]').forEach((button) => {
                    button.addEventListener('click', this.doBeforeQuickSearchProduct.bind(this));
                });
            }
        }
    }

    doBeforeQuickSearchProduct() {
        this.renderQuickSearchProductBlock();
    }
    ...
    renderQuickSearchProductBlock() {
        if (!this.searchResultsWidget.classList.contains('ajax-loaded')) {
            ...
            let url = block ? .getAttribute('data-collection'),
                limit = block ? .getAttribute('data-limit'),
            ...
            if (url != null && url != undefined) {
                $.ajax({
                    ...
                    success: (data) => {
                        if (url != '') {
                            const dataRes = this.handleResponse(data, limit);
                            block.querySelector('.products-grid').innerHTML = dataRes;
                        }
